...
//         }
//     }
//     const farePrice = (index: number) => {
//         return flightsData.map((ele, ind) => {
//             if (ind === index) {
//                 const roundedNumber = Math.ceil(ele[0].Fare.OfferedFare);
//                 const formattedNumber = roundedNumber.toLocaleString('en-IN', {
//                     style: 'currency',
//                     currency: 'INR',
//                 });
...
//         })
//     }
//     const farePrice1 = (price:number) => {
//         const roundedNumber = Math.ceil(price);
//         const formattedNumber = roundedNumber.toLocaleString('en-IN', {
//             style: 'currency',
//             currency: 'INR',
//         });
...
//         }
//     }
//  const flightRoute =(item:any)=>
//  {  
//     return item.map((ele:any, ind:any) => {
//         const DepTime = ele.Origin.DepTime;
//         const ArrTime = ele.Destination.ArrTime
//         return (
//             <View key={ele?.Airline.FlightNumber} >
...
//                     </View>
//                 </View>
//                 {item.length > 1 && ind <item.length - 1 && <View style={styles.layOver}><Text style={styles.layoverTiming}><Text style={styles.layoverTitle}>{`LayOver For :  `}</Text>{LayOverTime(item[ind + 1].GroundTime)} {`${ele.Destination.Airport.AirportCode}`}</Text></View>}
//             </View>
//         )
//     })
//  }
...
//                     </View>
//                 </View>
//                 <Text style={styles.farePrice}>{farePrice1(item.Fare.OfferedFare)}</Text>
//                 <TouchableOpacity style={styles.bookingButton}>
//                     <Text style={styles.bookingButtonText}>Book</Text>
//                 </TouchableOpacity>
//             </View>
...
const FlightDataCard:React.FC<IProps> = ({index}) => {
 const{flightsData}=useSelector((state:RootState)=>state.flightReducer)
 return (
    <View>
   { flightsData&&<FlatList data={flightsData[index]} renderItem={({item,index}:{item:any,index:number})=>
    {
        return(
            <FlightCard
            flightGrp={item}
